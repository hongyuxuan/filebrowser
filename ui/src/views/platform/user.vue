...
<el-drawer v-model="show" direction="rtl" size="600px">
  ...
  <template #footer>
    <div style="flex: auto">
      ...
      <el-button type="primary" @click="confirmClick(user)">提交</el-button>
    </div>
  </template>
</el-drawer>
</template>
...
const getList = async (page) => {
  ...
  let response = await axios.get(`/filebrowser/db/user?${url}`)
...
const confirmClick = async (f) => {
  ...
  await f.validate(async (valid) => {
    if(valid) {
      ...
      if(edit.value === false) {
        let password = await axios.post(`/filebrowser/auth/adduser`, params)
        getList(1)
        current.value = 1
        show.value = false
